...
const VideoSelector = ({ setVideos }) => {
  ...
  return (
    <StyledWrapper>
      <div id="container" className="video-selector"  > 
        ...
        <label htmlFor="file" className="footer"> 
          ...
          <p>{selectedCount > 0 ? `${selectedCount} video(s) selected` : 'No videos selected'}</p> 
        ...
        /> 
        {selectedCount > 0 && (
          <p className="selected-count">{selectedCount} videos loaded and ready to play</p>
        )}
      </div>
    </StyledWrapper>
  );
...
const StyledWrapper = styled.div`
  #container {
    ...
    padding: 10px;
    gap: 5px;
    background-color: rgba(11, 37, 89, 0);
  }
  ...
  .footer {
    background-color: rgba(10, 60, 125, 0.65);
    width: 100%;
    height: 40px;
    padding: 8px;
    border-radius: 10px;
